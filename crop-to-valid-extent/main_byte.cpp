...
struct LineCache {
  ...
  void AllocateRowsBuf() {
    if (rows_buf == nullptr) {
      rows_buf = new char[(long) cols * (long) kRowBufRowNum];
    }
  }
  ...
  char *ReadRow(int row_index, CPLErr &read_err) {
  ...
  void AllocateColsBuf() {
    if (cols_buf == nullptr) {
      cols_buf = new char[(long) rows * (long) kColBufColNum];
    }
    if (cols_buf_transpose == nullptr) {
      cols_buf_transpose = new char[rows * kColBufColNum];
    }
  }
  ...
  char *ReadCol(int col_index, CPLErr &read_err) {
    ...
    if (col_group != cur_col_group) {
      ...
      char *transpose_mem = cols_buf_transpose;
      for (int i = 0; i < col_num; ++i) {
        for (int j = 0; j < rows; ++j) {
          *transpose_mem++ = cols_buf[col_num * j + i];
        }
      }
      memcpy(cols_buf, cols_buf_transpose, rows * col_num * sizeof(char));
    }
 ...
 private:
  GDALRasterBand *band;
  char *rows_buf = nullptr;
  char *cols_buf = nullptr;
  char *cols_buf_transpose = nullptr;
  int rows;
  int cols;
  int cur_row_group;
  int cur_col_group;
...
bool CheckGridLineIsValid(LineCache &line_cache,
  ...
  char *line_buf = is_row ? line_cache.ReadRow(line_index, read_err) : line_cache.ReadCol(line_index, read_err);
  if (read_err != CE_None) {
    has_error = true;
    return false;
  }
